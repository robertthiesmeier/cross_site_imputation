...
On this site, we illustrate computer code to apply cross-site imputation as presneted in this paper (add link here) :page_facing_up:. All datasets are simulated and you can try out the code and implementation of the software package yourself. The results are not(!) identicial with the ones presneted in the paper as we are not allowed to share sensitive registry data.
